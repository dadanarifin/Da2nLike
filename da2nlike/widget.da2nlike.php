...
add_action( 'widgets_init', 'da2nlike_widget' );

// registers the widget
function da2nlike_widget() {
	register_widget( 'Da2nLike_Widget' );
} // end da2nlike_widget


class Da2nLike_Widget extends WP_Widget {
	...
	function Da2nLike_Widget() {
		...
		$control_ops = array('id_base' => 'da2nlike-widget' );

		// Create the widget
		$this->WP_Widget( 'da2nlike-widget', 'Most Liked Posts', $widget_ops, $control_ops );
	...
	WHERE post_status = 'publish' AND post_type = 'post' ORDER BY like_count DESC LIMIT $count");
		...
		if ($widget_posts):
			echo '<ul id="da2nlike_mostliked">';
			foreach ($widget_posts as $post):
				
				$da2nlike_disabled = get_post_meta($post->ID, 'da2nlike', true);
				if(!isset($da2nlike_disabled) || $da2nlike_disabled != 'disabled'):
		?>
			<li id="mostliked_post-<?php echo $post->ID ?>">
				...
				<div class="mostliked_post_meta">
					...
					<span class="mostliked_like_count">
						<?php echo $post->like_count . ' '; if(get_option('da2nlike_widget_txt')) echo get_option('da2nlike_widget_txt') ?>
					</span>
				</div>
