...
class da2nlike {
	...
	public function __construct($post_id, $user_id){
		...
		if(get_option('da2nlike_capabilities'))
			$this->capabilities = get_option('da2nlike_capabilities');
		
		if (get_option('da2nlike_style'))
			$this->style = get_option('da2nlike_style');
			
		if (get_option('da2nlike_colour'))
			$this->colour = get_option('da2nlike_colour');
			
		if (get_option('da2nlike_like_txt'))
			$this->like_txt = get_option('da2nlike_like_txt');
			
		if (get_option('da2nlike_unlike_txt'))
			$this->unlike_txt = get_option('da2nlike_unlike_txt');
		
		$this->likes_count();
	} // end da2nlike __construct
	...
	public function like_button(){
		...
		if(($this->capabilities === 'users-only' && $this->user_is_loggedin) || $this->capabilities === 'all'){
			if(!$this->check_recurring_like()){
				...
				// onclick javascript function for processing. found in da2nlike.js in /js folder
				$onclick_like = 'da2nlike(' . $this->post_id . ', ' . $this->user_id . ', \'like\');';
				
				// like button
				$button =  '<div class="da2nlike_button '.$this->style.' light_bg '.$this->colour.'" id="da2nlike-post-'.$this->post_id.'">' . "\n";
				$button .= "\t" . '<span class="da2nlike_icon" onclick="'.$onclick_like.'">&nbsp;</span>' . "\n";
				$button .= "\t" . '<span class="da2nlike_linebreaker first"></span>' . "\n";
				$button .= "\t" . '<span class="da2nlike_count">'.$this->likes_count.'</span>' . "\n";
				$button .= "\t" . '<span class="da2nlike_linebreaker last"></span>' . "\n";
				$button .= "\t" . '<span class="da2nlike_like_unlike" onclick="'.$onclick_like.'">'.$this->like_txt.'</span>' . "\n";
				$button .= '</div>' . "\n";
				
				
			} elseif($this->check_recurring_like()){
				// onclick javascript function for processing the unlike. found in da2nlike.js in /js folder
				$onclick_unlike = 'da2nlike(' . $this->post_id . ', ' . $this->user_id . ', \'unlike\')';
			
				// like button
				$button =  '<div class="da2nlike_button '.$this->style.' light_bg '.$this->colour.'" id="da2nlike-post-'.$this->post_id.'">' . "\n";
				$button .= "\t" . '<span class="da2nlike_icon" onclick="'.$onclick_unlike.'">&nbsp;</span>' . "\n";
				$button .= "\t" . '<span class="da2nlike_linebreaker first"></span>' . "\n";
				$button .= "\t" . '<span class="da2nlike_count">'.$this->likes_count.'</span>' . "\n";
				$button .= "\t" . '<span class="da2nlike_linebreaker last"></span>' . "\n";
				$button .= "\t" . '<span class="da2nlike_like_unlike" onclick="'.$onclick_unlike.'">'.$this->unlike_txt.'</span>' . "\n";
				$button .= '</div>' . "\n";
			...
			// count
			$button =  '<div class="da2nlike_button '.$this->style.' light_bg '.$this->colour.' modal" id="da2nlike-post-'.$this->post_id.'">' . "\n";
			$button .= "\t" . '<span class="da2nlike_icon">&nbsp;</span>' . "\n";
			$button .= "\t" . '<span class="da2nlike_linebreaker first"></span>' . "\n";
			$button .= "\t" . '<span class="da2nlike_count">'.$this->likes_count.'</span>' . "\n";
			$button .= '</div>' . "\n";
